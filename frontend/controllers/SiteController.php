...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $sliders = News::find()->where( "created_at BETWEEN DATE_ADD(NOW(), INTERVAL -6 hour) AND NOW() " )->orderBy( [ "cnt" => SORT_DESC ] )->limit( 8 )->all();
        foreach ($sliders as $item) {
            $notInIds[] = $item->id;
        }

        $topNews = News::find()->where( "created_at BETWEEN DATE_ADD(NOW(), INTERVAL -6 hour) AND NOW()  AND id NOT IN(" . implode( ",",
