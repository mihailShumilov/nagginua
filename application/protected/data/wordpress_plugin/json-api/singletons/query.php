...
class JSON_API_Query
{
    ...
    function get_controller()
    {
        ...
        if (preg_match('/^[a-zA-Z_]+$/', $json)) {
            ...
            if (preg_match('/^([a-zA-Z0-9_]+)(\/|\.)[a-zA-Z0-9_]+$/', $json, $matches)) {
                return $matches[1];
            } else {
                return 'core';
            }
    ...
    function get_legacy_controller($json)
    {
        ...
        if ($json == 'submit_comment') {
            if ($json_api->controller_is_active('respond')) {
                ...
                $json_api->error(
                    "The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > JSON API."
                );
            }
        } else {
            if ($json == 'create_post') {
                if ($json_api->controller_is_active('posts')) {
                    ...
                    $json_api->error(
                        "The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > JSON API."
                    );
                }
            } else {
                return 'core';
    ...
    function get_method($controller)
    {
        ...
        if (empty($method)) {
            ...
            if (method_exists("JSON_API_{$controller}_Controller", $method)) {
