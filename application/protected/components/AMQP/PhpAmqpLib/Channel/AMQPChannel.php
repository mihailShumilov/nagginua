<?php
	...
	class AMQPChannel extends AbstractChannel
	{
		...
		public function exchange_delete(
			...
			$ticket = $this->getTicket( $ticket );
			list( $class_id, $method_id, $args ) = $this->protocolWriter->exchangeDelete( $ticket, $exchange,
				$if_unused, $nowait );
			$this->send_method_frame( array( $class_id, $method_id ), $args );
		...
		public function exchange_unbind(
			...
			list( $class_id, $method_id, $args ) = $this->protocolWriter->exchangeUnbind( $ticket, $source,
		...
		public function queue_bind(
			...
			list( $class_id, $method_id, $args ) = $this->protocolWriter->queueBind( $ticket, $queue, $exchange,
				$routing_key, $nowait, $arguments );
		...
		public function queue_delete(
			...
			list( $class_id, $method_id, $args ) = $this->protocolWriter->queueDelete( $ticket, $queue, $if_unused,
				$if_empty, $nowait );
		...
		{
			list( $class_id, $method_id, $args ) = $this->protocolWriter->basicNack( $delivery_tag, $multiple,
				$requeue );
			$this->send_method_frame( array( $class_id, $method_id ), $args );
		}
