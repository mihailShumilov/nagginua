<?php
	...
	class Protocol091Test extends \PHPUnit_Framework_TestCase
	{
		...
		{
			$expected = "\x01/\x00";
			list( $class_id, $method_id, $args ) = $this->protocol091->accessRequest( "/", false, false, false, false,
				false );
			$this->assertEquals( $expected, $args->getvalue() );

			$expected = "\x04/foo\x00";
			list( $class_id, $method_id, $args ) = $this->protocol091->accessRequest( "/foo", false, false, false,
				false, false );
			$this->assertEquals( $expected, $args->getvalue() );
		}
		...
		public function testBasicConsume()
		{
			...
			list( $class_id, $method_id, $args ) = $this->protocol091->basicConsume( 0, 'foo', 'bar', false, false,
				false, false );
			$this->assertEquals( $expected, $args->getvalue() );
		}
